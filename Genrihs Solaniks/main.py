...
class Tree:
    ...
    def calculate_heuristic_values(self, node, ai_goes_first, minimax_chosen, alpha=float('-inf'), beta=float('inf')):
        if not node.children:
            ...
            node.heuristic_value = (-1 if final_score % 2 == 0 else 1) if ai_goes_first else (1 if final_score % 2 == 0 else -1)
        ...
        if minimax_chosen:
            if ai_goes_first:
                ...
                for child in node.children:
                    child_value, child_sum = self.calculate_heuristic_values(child, not ai_goes_first, minimax_chosen)
...
# =======================
# Функции для переходов между этапами
# =======================
...
def ai_move():
    ...
    for child in tree.root.children:
        ...
        if divisor is not None and child.heuristic_value > best_value:
...
# =======================
# Функции отрисовки экранов для разных этапов
# =======================

def draw_settings_algorithm():
    pygame.draw.rect(screen, (50, 50, 50), (150, 100, 500, 400))
    draw_text("Выберите алгоритм:", WIDTH//2, 150, 28, (255,255,255))
    draw_button("Minimax", WIDTH//2 - 150, 220, 120, 50, 
                (100,200,100) if minimax_chosen == True else (200,200,200), 
                (150,255,150), False, lambda: select_algorithm(True))
    draw_button("Alpha-Beta", WIDTH//2 + 30, 220, 120, 50, 
...
def draw_settings_first():
    pygame.draw.rect(screen, (50, 50, 50), (150, 100, 500, 400))
    draw_text("Выберите, кто ходит первым:", WIDTH//2, 150, 28, (255,255,255))
    draw_button("Игрок", WIDTH//2 - 150, 220, 120, 50, 
                (100,200,100) if ai_goes_first == False else (200,200,200), 
                (150,255,150), False, lambda: select_first(False))
    draw_button("ИИ", WIDTH//2 + 30, 220, 120, 50, 
...
def draw_choose_number():
    ...
    if choosing:
        ...
        draw_text("Выберите стартовое число:", WIDTH // 2, 100, 36)
